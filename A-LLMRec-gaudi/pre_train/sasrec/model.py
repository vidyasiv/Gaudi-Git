...
class SASRec(torch.nn.Module):
    def __init__(self, user_num, item_num, args):
        ...
        if self.nn_parameter:
            self.item_emb = nn.Parameter(torch.normal(0,0.1, size = (self.item_num+1, args.hidden_units)))
            self.pos_emb = nn.Parameter(torch.normal(0,0.1, size=(args.maxlen, args.hidden_units)))
        else:
            self.item_emb = torch.nn.Embedding(self.item_num+1, args.hidden_units, padding_idx=0)
            self.item_emb.weight.data.normal_(0.0,0.1)
